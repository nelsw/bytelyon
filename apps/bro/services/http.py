...
async def put_sitemap(c: AsyncClient, m: Sitemap) -> Response:
    return await c.put(
        url=f"{web_app_url}/api/bots/{m.bot_id}/sitemaps",
        data={
            "domain": m.domain,
            "urls": sorted(m.urls),
        },
...
async def put_search_page(c: AsyncClient, search_id: int, page: Page) -> Response:
    return await c.put(
        url=f"{web_app_url}/api/searches/{search_id}/page", data=asdict(page)
    )


async def put_sitemap_page(c: AsyncClient, sitemap_id: int, page: Page) -> Response:
    return await c.put(
        url=f"{web_app_url}/api/sitemaps/{sitemap_id}/page", data=asdict(page)
    )
