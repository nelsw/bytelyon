...
class SearchBot:
    ...
    @staticmethod
    def create_page(url:str, html:str, title:str, key:str, idx:int, kind:str)->dict:
        d = Doc(html)
    ...
    def handle_sponsored_products(self, context: BrowserContext, locators: list[Locator]) -> None:
        ...
        for loc in locators:
            ...
            if merchant_id is None:
                continue
            href = loc.locator(f"a[data-merchant-id=\"{merchant_id}\"]").get_attribute('href')
            if not href:
                continue
            ...
            try:
                ...
                title = p.title()
                self.sponsored_products.append(self.create_page(p.url, html, title, key, len(self.sponsored_products), "sponsored_product"))
            except Error as e:
    ...
    def handle_sponsored_results(self, context: BrowserContext, locators: list[Locator]) -> None:
        ...
        for loc in locators:
            ...
            try:
                ...
                p.screenshot(path=key, full_page=True)
                title = p.title()
                self.sponsored_results.append(self.create_page(p.url, html, title, key, len(self.sponsored_results), "sponsored_result"))
            except Error as e:
    ...
    def handle_organic_results(self, context: BrowserContext, locators: list[Locator]) -> None:
        ...
        for loc in locators:
            ...
            try:
                ...
                p.screenshot(path=key, full_page=True)
                title = p.title()
                self.organic_results.append(self.create_page(href, html, title, key, len(self.organic_results), "organic_result"))
            except Error as e:
    ...
    def handle_organic_products_v2(self, context: BrowserContext, p: Page) -> None:
        ...
        for idx, loc in enumerate(locators):
            ...
            u = p.locator('div[data-redirect-url]').first.get_attribute('data-redirect-url')
            if u is None:
                continue
            ...
            try:
                ...
                self.organic_products.append(
                    self.create_page(np.url, html, title, key, len(self.organic_products), "organic_product"))
            except Error as e:
        ...
        with SB(uc=True) as sb:
            ...
            with sync_playwright() as p:
                ...
                try:
                    ...
                    _ = page.goto("https://www.google.com/ncr", wait_until="domcontentloaded")
                    accept_cookies(page)
                    ...
                    with open(f"{self.output_dir}/serp.html", "w", encoding="utf-8") as f:
                        f.write(page.content())

                    self.handle_organic_results(context, page.locator('h3[id]').all())
                    self.handle_sponsored_results(context, page.locator("[data-pcu]").all())
                    self.handle_sponsored_products(context, page.locator("[data-dtld]").all())
