#!/bin/sh
# Dual-mode entrypoint for the SeleniumBase Lambda image. Verbatim copy of
# ../../lambda/base-image/lambda-entrypoint.sh's logic — generic Lambda/X11
# glue, nothing CloakBrowser-specific, so it's reused as-is here.
#
#   1. Always start Xvfb on :99 so headed Chromium would work no matter how
#      the container is invoked (no current handler uses headed mode, but
#      this keeps the same invocation surfaces as every other image here).
#   2. Detect whether the CMD looks like a Lambda handler (a single
#      `module.func`-shaped argument). If yes, route through the Lambda
#      runtime client (using the bundled aws-lambda-rie locally, or talking
#      to the real Lambda Runtime API when AWS_LAMBDA_RUNTIME_API is set in
#      production).
#   3. Otherwise exec the CMD directly (`python`, `bash`, etc.).
set -e
...
# no leading dot). `python`, `bash`, etc. all fail this test and pass through
# to plain exec.
if [ $# -eq 1 ] && \
