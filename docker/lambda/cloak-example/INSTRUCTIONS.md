# CloakBrowser on AWS Lambda (vendor baseline — unmodified)

> **bytelyon note**: This directory is a verbatim copy of CloakHQ's official
> `examples/integrations/aws_lambda/` (fetched 2026-09-15 from
> https://github.com/CloakHQ/CloakBrowser). It is kept **deliberately
> unmodified** as a clean reference baseline to compare against
> `../base-image` + `../serp-scraper`, which fork from the same starting
> point but add GeoIP DB baking, real MS fonts, a Google-SERP-specific
> handler with block/CAPTCHA detection, and proxy retry/sticky-session
> logic. Deploy this as its own Lambda function (e.g. `bytelyon-cloak-example`)
> — do not overwrite `bytelyon-serp-scraper`.
...
> **bytelyon note**: our other Lambda images build with `--provenance=false --sbom=false` because AWS Lambda's image deploy rejects OCI manifests with those attestations. Add the same flags here when building for actual deployment (see the header comment in `Dockerfile`).
